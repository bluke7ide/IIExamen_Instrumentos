...
De manera explícita, los valores que toma el precio de la acción a lo largo de 5 años corresponden a los siguientes:

```{r}
S
```

El precio promedio esperado de la acción dadas las simulaciones corresponde a:

```{r}
mean(S)
```

Mientras que el precio determinado por la rentabilidad esperada de 9\% es:
...
plot(1:(t * (1 / dt)), S[, 1], type = "l", col = rgb(0, 0, 1, 0.2), lwd = 0.5,
     xlab = "Tiempo (meses)", ylab = "Precio de la acción",
     main = "Trayectorias simuladas del precio de la acción",
     ylim = range(S))  # Ajustar el rango del eje y

for (j in 2:n_sim) {
  lines(1:(t * (1 / dt)), S[, j], col = rgb(0, 0, 1, 0.2), lwd = 0.5)
}

# Resaltar la trayectoria promedio
lines(1:(t * (1 / dt)), rowMeans(S), col = "red", lwd = 2, lty = 2)
legend("topright", legend = c("Simulaciones individuales", "Promedio"),
       col = c(rgb(0, 0, 1, 0.5), "red"), lty = c(1, 2), lwd = c(1, 2))
```
